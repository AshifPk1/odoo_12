...
class SaleOrder(models.Model):
    ...
    promo_price = fields.Float(compute="_amount_all", string="Promotion", copy=False)
    ...
    def onchange_promotion_order_line(self):
        ...
        for order in self:
            if order.promotion_id:
                raise UserError(_(
                    "You can not change the order after promotion applied in order.\n If you want to change the order then first remove promotion."))
        return {}
    ...
    def find_promo(self):
        ...
        if self.coupon_code:
            print(self.coupon_code,'self.coupon_code')
            if len(self.coupon_code) != 10:
                ...
                promotion = self.env['promotion.coupon'].sudo().search(
                    [('code', '=', self.coupon_code), ('partner_id', '=', self.partner_id.id),
                     ('used', '=', False)]).promotion_id
                if promotion:
                    self.apply_promotion(promotion)
                else:
                    self.message_post(body=(
                        "<b>Coupon Code Does not match to any promotion or you are not applicable for this coupon or coupon use limit goen to maximum.</b>"))
            else:
    ...
    def _promotion_unset(self):
        ...
        if self.coupon_code:
            if len(self.coupon_code) != 10:
                ...
                self.env['promotion.coupon'].search(
                    [('promotion_id', '=', self.promotion_id.id), ('order_id', '=', self.id)]).unlink()
            else:
    ...
    def apply_promotion(self, promotion):
        ...
        for order in self:
            ...
            if order.order_line:
                ...
                if price_unit < 0.0:
                    if -(price_unit) <= order.amount_untaxed or promotion.compute_price == 'bogo_sale':
                        ...
                        if price_unit > 0.0 and promotion.max_promotion_amount != 0.0:
                            order.check_max_promotion(promotion)
                            order.message_post(body=("<b>Promotion applied successfully</b>"))
                            return True
                    ...
                    if price_unit > 0.0 and promotion.max_promotion_amount != 0.0:
                        ...
                        order.message_post(body=("<b>Promotion applied successfully</b>"))
                        return True
                    elif promotion.used_in_next_order and len(order.coupon_code) != 10:
                        coupon = promotion.applied_on_next_order(order)
                        return True
                    elif not price_unit > 0.0:
                        order._promotion_unset()
                ...
                order._promotion_unset()
                print('else order.order_line:')
                order.message_post(body=("<b>Order not Contain Promotion Criteria product.</b>"))

    def check_max_promotion(self, promotion):
        total = 0.0
        for line in self.order_line:
            amount = (line.product_uom_qty * line.price_unit * 100) / line.discount
            total += amount
        if total - self.amount_untaxed > promotion.max_promotion_amount:
        ...
        for line in self.order_line:
            ...
            orderline = self.env['sale.order.line'].search(
                [('order_id', '=', self.id), ('product_id', '=', line.product_id.id),
                 ('product_uom', '=', line.product_uom.id), ('discount', '=', line.discount)])
...
class SaleorderLine(models.Model):
    ...
    def onchange_promotion_order_line(self):
        promoline = self.env['sale.order.line'].search(
            [('order_id', '=', self.order_id.id), ('is_promotion', '=', True)]).write(
            {'price_unit': 0.0, 'product_uom_qty': 0.0})
